...
# Thank you for ensuring all prerequisites are met. You can now proceed with using the Chemical Condition of Use Tracker (CCUT).
...
def generate_report_and_diagram(qualitative_summary_file, casrn):
    ...
    with Diagram("", show=False, direction="TB", filename=diagram_path, outformat="png", graph_attr=graph_attr):
        ...
        process_sections = {}
        with Cluster("Process or Use Operation", graph_attr={**cluster_attr, "style": "filled", "bgcolor": "lightgreen"}):
        ...
        with Cluster("Consumer and Commercial Product Use Category", graph_attr={**cluster_attr, "style": "filled", "bgcolor": "lightseagreen"}) as cc_product_use_category_cluster:
        ...
        # NAICS Cluster
        naics_sections = {}
        with Cluster("NAICS", graph_attr={**cluster_attr, "style": "filled", "bgcolor": "lightgoldenrodyellow"}):
...
def generate_simplified_diagram(final_merged_file, casrn):
    ...
    def create_section_node(label):
        split_label_text = split_label(label, max_length=20)
        width = max(len(line) for line in split_label_text.split('\n')) * 0.2  # Adjust based on text length
        height = (split_label_text.count('\n') + 1) * 0.5  # Adjust based on line count
        return Custom(split_label_text, "./section_icon.png", fontsize="25", width=str(width), height=str(height), style="filled", fillcolor="white")
    ...
    graph_attr = {
        ...
        "ranksep": "4",  # Adjust rank separation
        "fontsize": "60",  # Font size for the diagram
        "fontname": "Arial-BoldMT",  # Bold font for the diagram
        "labeljust": "c",  # Center the label
        "labelloc": "t",  # Place the label at the top
    ...
    node_attr = {
        "fontsize": "60",  # Font size for FRS nodes
        "fontname": "Arial-BoldMT",  # Font for FRS nodes
    ...
    cluster_attr = {
        "fontsize": "60",  # Font size for cluster labels
        "fontname": "Arial-BoldMT",  # Bold font for cluster labels
        "fontcolor": "black",  # Font color
    ...
    with Diagram("", show=False, direction="TB", filename=simplified_diagram_path, outformat="png", graph_attr=graph_attr, node_attr=node_attr):
        ...
        with Cluster("Manufacturing/Import", graph_attr={**cluster_attr, "bgcolor": "lightblue"}) as manufacturing_import_cluster:
        ...
        # Process or Use Operation Cluster
        process_sections = {}
        with Cluster("Process or Use Operation", graph_attr={**cluster_attr, "bgcolor": "lightgreen"}) as process_cluster:
            for col in df_process.columns:
        ...
        # Function Cluster
        function_sections = {}
        with Cluster("Function", graph_attr={**cluster_attr, "bgcolor": "lightcyan"}) as function_cluster:
        ...
        # NAICS Cluster
        naics_sections = {}
        with Cluster("NAICS", graph_attr={**cluster_attr, "bgcolor": "lightgoldenrodyellow"}) as naics_cluster:
            for col in df_naics_combined.columns:
        ...
        # Product Use Information Cluster
        product_use_sections = {}
        with Cluster("Product Use Information", graph_attr={**cluster_attr, "bgcolor": "lightgrey"}) as product_use_cluster:
        ...
        # End-of-Life Cluster
        end_of_life_sections = {}
        with Cluster("Industrial End-of-Life", graph_attr={**cluster_attr, "bgcolor": "lightpink"}) as end_of_life_cluster:
...
╔═══════════════════════════════════════════════════════════════════════╗
║                                                                       ║
║       Welcome to the Chemical Condition of Use Tracker (CCUT)         ║
║                             Version 1.4                               ║
